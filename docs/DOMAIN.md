...
| **Last-run version** | The app version seen at the previous launch, persisted in `settings.json`; compared to the running `app.getVersion()` on startup to fire the one-time post-update notification. | [types.ts#AppSettings](../src/types.ts), [settings.ts](../src/settings.ts), [main.ts](../src/main.ts) |
...
| **Update state** | The electron-updater lifecycle snapshot the tray renders into its single update row: `status` (`idle`/`checking`/`available`/`downloading`/`downloaded`/`error`), `version`, `percent`, `error`. Pushed by the UpdateService on every transition to the tray (row **+** icon badge) and the UpdateNotifier. | [types.ts#UpdateState](../src/types.ts), [update-service.ts](../src/update-service.ts), [ADR-011](./adr/011-auto-update-mechanism.md) |
| **Update badge** | A colored dot with a white action glyph composited onto the menu-bar icon only while an update needs action — blue + up-arrow = download available, orange + restart arrow = restart ready. The committed asset stays a template; the badged variant is a runtime non-template composite. | [tray-icon.ts](../src/tray-icon.ts), [tray.ts](../src/tray.ts), [ADR-011 amendment](./adr/011-auto-update-mechanism.md#amendment-attention-cues-2026-07) |
| **Stats card** | The rich bitmap the menu shows in place of plain text rows: a 2×2 today/30-day stat grid, a warm bar chart, "Top model", and a footnote, on a transparent background. Rasterized off-screen, one frame at a time, by `MenuCardRenderer` + `CardAnimator`; display-only (the "Open Usage Dashboard…" row beneath it is the drill-down). Values/bars render directly at their final state; ember particles drift while the menu is open (an odometer-roll and a bar-growth animation were removed — see [ADR-013's amendment](./adr/013-menu-card-animation-framework.md#amendment-the-verification-item-resolved-false-2026-07)). | [menu-card-window.ts](../src/menu-card-window.ts), [card-animator.ts](../src/card-animator.ts), [src/menu-card/card.ts](../src/menu-card/card.ts), [ADR-009](./adr/009-menu-stats-card.md), [ADR-013](./adr/013-menu-card-animation-framework.md) |
