...
| **Stats card** | The rich bitmap the menu shows in place of plain text rows: a 2×2 today/30-day stat grid, a warm bar chart, "Top model", and a footnote, on a transparent background. Rasterized off-screen, one animation frame at a time, by `MenuCardRenderer` + `CardAnimator`; display-only (the "Open Usage Dashboard…" row beneath it is the drill-down). Animates on value change (odometer roll), on data reveal (bar growth), and while the menu is open (ember particles). | [menu-card-window.ts](../src/menu-card-window.ts), [card-animator.ts](../src/card-animator.ts), [src/menu-card/card.ts](../src/menu-card/card.ts), [ADR-009](./adr/009-menu-stats-card.md), [ADR-013](./adr/013-menu-card-animation-framework.md) |
