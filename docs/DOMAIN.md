...
| **Stats card** | The rich bitmap the menu shows in place of plain text rows: a 2×2 today/30-day stat grid, a warm bar chart, "Top model", and a footnote, on a transparent background. Rasterized off-screen by `MenuCardRenderer`; display-only (the "Open Usage Dashboard…" row beneath it is the drill-down). No animation — an odometer-roll, a bar-growth reveal, and ember particles were all tried and removed (see [ADR-013](./adr/013-menu-card-animation-framework.md) and its amendments); values/bars render directly at their final state. | [menu-card-window.ts](../src/menu-card-window.ts), [src/menu-card/card.ts](../src/menu-card/card.ts), [ADR-009](./adr/009-menu-stats-card.md), [ADR-013](./adr/013-menu-card-animation-framework.md) |
