...
- **Menu Card** ([menu-card.stories.ts](../stories/menu-card.stories.ts)) — drives the **real** `drawCard` function from [src/menu-card/card.ts](./modules/menu-card.md), no Electron, no hidden window: a single `Default` story with "Bump values" / "New 30-day data" / "Toggle theme" buttons that each trigger an instant re-render. The card has no animation of any kind — three attempts (an odometer digit-roll, a bar-growth reveal, ember particles — issues #52/#53/#54) were built and then all removed, because Electron never repaints a `MenuItem`'s icon while the native tray dropdown is already open — see [ADR-013](./adr/013-menu-card-animation-framework.md) and its amendments for the full trace.
...
- [modules/menu-card.md](./modules/menu-card.md), [adr/013](./adr/013-menu-card-animation-framework.md).
