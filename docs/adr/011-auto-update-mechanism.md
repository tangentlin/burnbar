...
- **Tray-icon badge.** When an update needs attention the menu-bar icon gains a colored dot with a small white **action glyph** inside it, distinct per pending action (blue dot + up-arrow = download available, orange dot + restart arrow = restart ready) so both color and shape convey the state; every other state shows the plain icon. The committed asset stays a monochrome **template** ([ADR-004](./004-template-tray-icon.md)); the badged variant is composited at runtime into a **non-template** image (recolor the glyph for the current appearance + stamp the dot + the action glyph), so ADR-004's template contract is unchanged for the default icon. — [tray-icon.ts](../../src/tray-icon.ts), [tray.ts](../../src/tray.ts).
...
- (−) The default tray icon stays a template, but the *badged* variant is a runtime-composited non-template image, so its light/dark correctness is handled in code (keyed off the menu bar's real appearance — see the [reliable-detection amendment](#amendment-reliable-menu-bar-appearance-detection-2026-07) below) rather than by macOS auto-tinting.
...
## Amendment: reliable menu-bar appearance detection (2026-07)

The badge amendment above keyed the glyph recolor off `nativeTheme.shouldUseDarkColors`. In practice this produced a black (light-appearance) glyph on a dark menu bar: `nativeTheme.shouldUseDarkColors` tracks the app's own UI theme, not the tray specifically, and is documented as unreliable for the tray — [electron/electron#25478](https://github.com/electron/electron/issues/25478) ("nativeTheme.shouldUseDarkColors is only for setting theme of our APP interface and not for tray icon") and [#21899](https://github.com/electron/electron/issues/21899) (the value can get stuck on macOS Catalina+). Burnbar is especially exposed: a windowless, Dock-hidden (`LSUIElement`) app has no window to keep the native theme observer reliably in sync. The compositor math itself was verified correct against the real committed asset (see [test/tray-icon.test.ts](../../test/tray-icon.test.ts)) — the bug was entirely in the signal feeding it.

- **Source of truth moved to the OS setting AppKit itself reads**: `defaults read -g AppleInterfaceStyle`, the same `NSUserDefaults` key that drives real menu-bar tinting — not `nativeTheme`. New module [appearance.ts](../../src/appearance.ts) (`detectAppearance`), Electron-free and dependency-injected like [capture.ts](../../src/capture.ts)'s `CcusageRunner`, so it stays unit-testable under plain Node ([vitest.config.ts](../../vitest.config.ts) runs no Electron runtime). `nativeTheme.shouldUseDarkColors` survives only as the fallback when `defaults` itself can't run (non-macOS, a sandboxed test environment) — injected by the caller, not read inside the pure module.
- **`nativeTheme` "updated" is now a trigger, not a value source.** `TrayManager` caches the detected appearance in `this.appearance` and re-detects (async, via `refreshAppearance()`) on cold start, on every update-state transition (`renderUpdate`), and on `nativeTheme` "updated" — cheap and infrequent, never per animation frame. Both the badge glyph and the stats card's value-text color ([ADR-009](./009-menu-stats-card.md)) now read from this same corrected field.

### Amended consequences

- (+) The badge glyph and card text now match the real menu bar in the case that motivated this amendment (system-wide Dark Mode, cold-started tray app) without relying on an Electron API documented as unsuited to the tray.
- (−) `refreshAppearance()` shells out to `defaults` (an async child process) on each of its three trigger points. Bounded and infrequent by design — not a concern in practice — but it is a real process spawn where `nativeTheme.shouldUseDarkColors` was a synchronous property read.
- (−) Does not cover macOS's wallpaper-driven menu-bar auto-tinting (Big Sur+, independent of the system-wide Dark Mode toggle — see electron/electron#25478) since there's no `nativeTheme` event for that transition to trigger a re-detect on; out of scope for the reported bug (a straightforward Dark Mode toggle).
