...
### Views
A stacked bar chart over a continuous daily axis. Total = combined cost per day; By model / By agent = one stacked series each. The header shows the range label and its summed cost. — [src/dashboard/renderer.ts](../../src/dashboard/renderer.ts), [derive.ts](../../src/derive.ts)
...
- [ ] 30d / 90d / All presets re-scope the chart and the headline total. — [src/dashboard/renderer.ts](../../src/dashboard/renderer.ts)
