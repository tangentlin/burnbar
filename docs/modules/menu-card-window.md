...
`render(data: MenuCardData): Promise<NativeImage | null>` — renders the stats card for the given data. `renderIcon(name: "refresh" | "dashboard"): Promise<NativeImage | null>` — a menu-row glyph returned as a **template** image (`setTemplateImage(true)`, so macOS tints it). Both return `null` on any failure (the tray falls back gracefully). `dispose()` destroys the hidden window. Module-private: `rasterize()` (the shared eval-and-decode step both public methods delegate to), `liveContents()` (the shared "window is up, hand back its webContents" guard), `ensureWindow()` (lazy, once), and the `SCALE` / data-URL-prefix constants.
...
- Drive the page's draw globals via `webContents.executeJavaScript`: `render` calls `__burnbarDrawCard(data)`, `renderIcon` calls `__burnbarDrawIcon(name)`, both through the shared `rasterize` helper that decodes a PNG data-URL string into a `NativeImage` tagged `scaleFactor: SCALE` (crisp on retina menus); `renderIcon` additionally flags the result a **template** image. — [render](../../src/menu-card-window.ts), [renderIcon](../../src/menu-card-window.ts), [rasterize](../../src/menu-card-window.ts)
- Be best-effort: guard a destroyed window, validate the returned payload shape, and swallow errors into a `null` (logged) so a render failure never crashes the tray. — [rasterize](../../src/menu-card-window.ts)
- Destroy the window on `dispose` (called by `main` at quit). — [dispose](../../src/menu-card-window.ts)

## Non-Goals

- **No drawing** — the canvas layout, fonts, colors, and number formatting live in the browser-context [menu-card](./menu-card.md) page. This module only shuttles one `(data)` request per call and decodes the result.
- **No animation, no frame scheduling** — the card has none (see [ADR-013](../adr/013-menu-card-animation-framework.md) for what was tried and removed); `render` is a plain one-shot call.
...
- No caching — the [tray](./tray.md) decides when to call `render` (it already skips unchanged data signatures before calling in); this module renders every time it's asked.

## How It Works

The first call (the tray's startup `loadIcons`, or the first card render) creates the hidden window and `loadFile`s `dist/menu-card/index.html`; `ensureWindow` memoizes the load promise so subsequent renders reuse the same window (cold render ≈ window-create + page-load, ~0.5s; warm renders are a few ms). `render`/`renderIcon` both go through `rasterize(expression)`, which evaluates the expression in the page's main world (reachable by `executeJavaScript` even with `contextIsolation` on, because that option isolates the *preload*, not injected evaluations), validates the result is a PNG data-URL string, decodes the base64 payload, and wraps it via `nativeImage.createFromBuffer(png, { scaleFactor: SCALE })`. `renderIcon` additionally calls `setTemplateImage(true)` so the glyph tints to the menu foreground.
...
- **Null-on-failure contract**: a destroyed window, a malformed/missing payload, or a thrown error all yield `null`; the [tray](./tray.md) renders a plain-text fallback the first time the image is null. — [rasterize](../../src/menu-card-window.ts)
...
- [tray.ts](../../src/tray.ts) → [tray.md](./tray.md) — the sole caller: calls `render`/`renderIcon`, caches the resulting `NativeImage`, and attaches it.
...
- [adr/009-menu-stats-card.md](../adr/009-menu-stats-card.md) — why a hidden-window canvas instead of a template sparkline or a native renderer. [adr/013-menu-card-animation-framework.md](../adr/013-menu-card-animation-framework.md) — the animation framework that was tried on top of this module and later fully reverted.
