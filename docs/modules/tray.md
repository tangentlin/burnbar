...
flowchart LR
    ...
    themeEvt["nativeTheme 'updated'"] --> refreshApp
    refreshApp --> detect["appearance.ts#detectAppearance()"]
    detect --> setAppearance["this.appearance = result"]
    setAppearance --> refresh
    setAppearance --> icon["refreshTrayIcon() (badge, keyed by appearance)"]
```
